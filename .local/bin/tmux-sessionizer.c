...
char *run_fzf(char **items, int count) {
    ...
    FILE *fzf = popen("cat /tmp/fzf_input_c | fzf --layout=reverse --tmux center --ansi", "r");
    if (!fzf) {
...
void add_folder(const char *name, const char *path) {
    if (num_folders >= MAX_FOLDERS) return;
...
int main() {
    ...
    for (int i = 0; i < num_projects; i++) {
        if (projects[i].type == SINGLE) {
            ...
            while ((entry = readdir(dir)) != NULL) {
                ...
                int path_len = snprintf(full_path, sizeof(full_path), "%s/%s", projects[i].path, entry->d_name);
                if (path_len >= MAX_PATH) continue; // Skip if path is too long
    ...
    if (!selected || strlen(selected) == 0) return 0;

    // Strip prefix - 🟢 is 4 bytes in UTF-8, plus space = 5 bytes
    char *name = selected;
    if (strncmp(name, "🟢 ", 5) == 0) name += 5;
    else if (strncmp(name, "   ", 3) == 0) name += 3;
    ...
    if (!in_tmux && !tmux_running) {
        snprintf(cmd, sizeof(cmd), "tmux new-session -s %s -c '%s'", selected_name, selected_path);
        system(cmd);
        return 0;
    }

    if (!tmux_session_exists(selected_name)) {
        snprintf(cmd, sizeof(cmd), "tmux new-session -ds %s -c '%s'", selected_name, selected_path);
        system(cmd);
    }
