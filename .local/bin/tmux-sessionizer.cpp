...
bool is_tmux_running() { return system("pgrep tmux >/dev/null 2>&1") == 0; }
...
std::string run_fzf(const std::vector<std::string> &items) {
  ...
  FILE *fzf = popen(
      "cat /tmp/fzf_input_cpp | fzf --layout=reverse --tmux center --ansi",
      "r");
  if (!fzf) {
...
int main() {
  ...
  for (const auto &project : projects) {
    if (project.type == ProjectType::Single) {
      ...
      folders[name] = {project.path, active_sessions.count(session_name) > 0};
    } else {
      for (const auto &entry : read_directory(project.path)) {
        ...
        folders[entry] = {full_path, active_sessions.count(session_name) > 0};
      }
    }
  }
